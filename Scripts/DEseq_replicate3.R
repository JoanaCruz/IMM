...
#
analysis=c("rep1_lane1.vs.rep1_lane2","rep2_lane1.vs.rep2_lane2","rep1.vs.rep2","public_rep.vs.control", "rep1.vs.control","rep2.vs.control")

for ( cond in analysis){

	# define data design & table with counts
	if (cond=="rep1_lane1.vs.rep1_lane2"){
		dataDesign = data.frame(
			row.names = c( colnames( count_table )[2], colnames( count_table )[3]),
			condition = c( colnames( count_table )[2], colnames( count_table )[3]),
	...
	else if (cond=="rep2_lane1.vs.rep2_lane2"){
		dataDesign = data.frame(
			row.names = c( colnames( count_table )[4], colnames( count_table )[5]),
			condition = c( colnames( count_table )[4], colnames( count_table )[5]),
			libType = rep("paired-end",2))
		count_table_analysis = count_table[,c( colnames( count_table )[4], colnames( count_table )[5])]
		methodology='blind'
		sharingM='fit-only'
		fitT='local'
	}
	else if (cond=="rep1.vs.rep2"){
		dataDesign = data.frame(
			row.names = c( colnames( count_table )[2], colnames( count_table )[3], colnames( count_table )[4], colnames( count_table )[5]),
			condition = c( "rep1", "rep1", "rep2", "rep2"),
			libType = rep("paired-end",4))
		count_table_analysis = count_table[,c( colnames( count_table )[2], colnames( count_table )[3], colnames( count_table )[4], colnames( count_table )[5])]
		methodology='pooled'
		sharingM='maximum'
		fitT='parametric'
	}
		...
		dataDesign = data.frame(
			...
			condition = c( "Control_HeLa", rep("rep",4) ),
			libType = rep("paired-end",5))
		count_table_analysis = count_table[,c( colnames( count_table )[1], colnames( count_table )[2], colnames( count_table )[3], colnames( count_table )[4], colnames( count_table )[5])]
		...
		dataDesign = data.frame(
			row.names = c( colnames( count_table )[1], colnames( count_table )[4], colnames( count_table )[5]),
			condition = c( "Control", "rep2", "rep2"),
			libType = rep("paired-end",3))
		count_table_analysis = count_table[,c( colnames( count_table )[1], colnames( count_table )[4], colnames( count_table )[5])]
		methodology='pooled'
		sharingM='maximum'
		fitT='local'
	}
