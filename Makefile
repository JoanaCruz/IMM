...
sorted: $(addprefix $(SORTED_DIR)/,$(addsuffix _sorted.sam.gz, $(basename $(basename $(wildcard $(READS_DIR)/*.fq.gz)))))
	
sam: $(addprefix $(SAM_DIR)/,$(addsuffix .sam.gz, $(basename $(basename $(wildcard $(READS_DIR)/*.fq.gz)))))
