...
GTF_FILE := $(AREF)/Homo_sapiens.GRCh37.68_chr.gtf

all: R

R: $(addprefix $(DESEQ_DIR)/,$(subst LM1, DESeq, $(notdir $(subst L1_1.fq.gz,HeLa.txt.gz, $(wildcard $(READS_DIR)/LM1_*_1.fq.gz)))))
...
$(COUNTS_DIR)/LM_%_HeLa.txt.gz : $(COUNTS_DIR)/LM1_%_L1.txt.gz $(COUNTS_DIR)/LM2_%_L1.txt.gz 
	@ gunzip $(COUNTS_DIR)/LM*_$*_L1.txt.gz
	@ join -t "`/bin/echo -e '\t'`" --head $(COUNTS_DIR)/LM1_$*_L1.txt $(COUNTS_DIR)/LM2_$*_L1.txt > $(COUNTS_DIR)/LM_$*_HeLa.txt
	@ sed -i '1s/^/LM1_'$*'\tLM2_'$*'\n/' $(COUNTS_DIR)/LM_$*_HeLa.txt
	@ gzip $(COUNTS_DIR)/*.txt


# Do differencial expression analysis (using DESeq)
$(DESEQ_DIR)/DESeq_%_HeLa.txt.gz: $(COUNTS_DIR)/LM_%_HeLa.txt.gz | $(DESEQ_DIR)/.d
	@ gunzip $(COUNTS_DIR)/LM_$*_HeLa.txt.gz
	@ Rscript $(MAKE_DIR)/DEseq.R $(COUNTS_DIR)/LM_$*_HeLa.txt $* $(DESEQ_DIR)/
	@ gzip $(DESEQ_DIR)/DESeq_$*_HeLa.txt
