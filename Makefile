...
$(SAM_DIR)/%.sam.gz :  $(TRIM_DIR)/%_1_trimmed.gz $(TRIM_DIR)/%_2_trimmed.gz | $(SAM_DIR)/.d
	$(BOWTIE_DIR)/bowtie2 -x $(AREF)/Homo_sapiens.1 --sensitive -1 $(TRIM_DIR)/$*_1_trimmed.gz -2 $(TRIM_DIR)/$*_2_trimmed.gz | gzip > $(MAKE_DIR)/$(SAM_DIR)/$*.sam.gz
	@ echo "Alignment done."
...
$(COUNTS_DIR)/%.txt.gz : $(PAIRED_DIR)/%_paired.sam.gz $(GTF_FILE) | $(COUNTS_DIR)/.d
	...
	@ python -m HTSeq.scripts.count $(PAIRED_DIR)/$*_paired.sam $(AREF)/*.gtf > $(COUNTS_DIR)/$*.txt
	@ gzip $(PAIRED_DIR)/*.sam
	@ gzip $(COUNTS_DIR)/*.txt
